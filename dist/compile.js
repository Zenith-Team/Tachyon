...
export let oFile;
export let symbolMap;
const cwd = process.cwd();
const { positionals: targets, values: { project: projectPathRaw, threads: threadsRaw, meta: metaFolderName, ghs: ghsPathRaw, out: outPathRaw, aflag, cflag, bflag, lflag, rpx: produceRPXFlag, typf: produceTYPF, console: consoleFlag, 'no-cache': noCache } } = util.parseArgs({
...
if (targets.length > 1)
    abort('Multiple targets are not yet supported.');
...
if (threads > os.cpus().length)
    abort(`Number of threads exceeds number of available CPU cores (${os.cpus().length}).`);
const outPath = outPathRaw ? (['.rpx', '.rpl', '.elf'].includes(path.extname(outPathRaw).toLowerCase()) ?
    abort('Output path may not contain the file extension, only the name.') : path.resolve(cwd, outPathRaw)) : null;
...
if (!fs.existsSync(projectPath))
    abort('Project path folder does not exist!');
if (!fs.existsSync(metaPath))
    abort(`Project meta folder not found: ${metaPath}`);
if (!fs.existsSync(path.join(metaPath, 'project.yaml')))
    abort('Project meta folder does not have a project.yaml!');
if (!fs.existsSync(path.join(metaPath, 'syms')))
    abort('Project meta folder does not have a "syms" folder!');
if (!fs.existsSync(path.join(metaPath, 'syms', 'main.map')))
    abort('Project symbols folder does not have a main.map file!');
if (!fs.existsSync(path.join(metaPath, 'conv')))
    abort('Project meta folder does not have a "conv" folder!');
if (!fs.existsSync(path.join(metaPath, 'linker')))
    fs.mkdirSync(path.join(metaPath, 'linker'));
...
const validConsoleOutputs = ['cafeloader', 'cf', 'none'];
let consoleOutput = 'none';
switch (consoleFlag.toLowerCase()) {
    case 'cf':
    case 'cafeloader':
...
if (consoleOutput !== 'none')
    console.warn('Console output enabled, this is experimental!');
...
if (!fs.existsSync(project.rpxDir))
    abort(`RPX folder ${project.rpxDir} does not exist!`);
if (!fs.existsSync(baseRpxPath))
    abort(`Base RPX ${project.targetBaseRpx}.rpx for target ${target} does not exist!`);
if (!fs.existsSync(project.includeDir))
    abort(`Include folder ${project.includeDir} does not exist!`);
if (!fs.existsSync(project.sourceDir))
    abort(`Source folder ${project.sourceDir} does not exist!`);
if (!fs.existsSync(project.modulesDir))
    abort(`Modules folder ${project.modulesDir} does not exist!`);
...
else if (!fs.existsSync(objsPath))
    fs.mkdirSync(objsPath);
...
if (gbuild.error || gbuild.signal || gbuild.stderr || gbuild.status !== 0)
    abort('gbuild command failed!');
...
const asmCache = fs.existsSync(asmCachePath) ? JSON.parse(fs.readFileSync(asmCachePath, 'utf8')) : {};
const depCache = {};
for (const asmfile of project.asmFiles) {
    ...
    if (deps)
        for (const dep of deps)
            depCache[dep] = fs.statSync(dep).mtimeMs;
    ...
    console.log('Assembling', asmfile, modifiedDep ? `because ${path.relative(project.sourceDir, modifiedDep)} has changed` : '');
    const asppcArgs = [
    ...
    if (asppc.error || asppc.signal || asppc.stderr || asppc.status !== 0)
        abort('asppc command failed!');
}
