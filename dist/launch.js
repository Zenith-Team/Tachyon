...
export async function launch() {
    ...
    const { positionals: [rpxPathRaw], values: { cemu, game, fullscreen, allow, block } } = util.parseArgs({
    ...
    const logTypes = new Set(['all', 'time', 'init', 'iosu', 'vulkan', 'osconsole', 'osprefix', 'crashlog']);
    const allowedLogs = new Set(allow);
    const blockedLogs = new Set(block);
    if (allowedLogs.size && blockedLogs.size)
        abort('Cannot allow and block logs at the same time.');
    allowedLogs.forEach(a => { if (!logTypes.has(a))
        abort(`Unknown allowed log type: ${a} (Must be one of: ${[...logTypes.values()].join(', ')})`); });
    blockedLogs.forEach(b => { if (!logTypes.has(b))
        abort(`Unknown blocked log type: ${b} (Must be one of: ${[...logTypes.values()].join(', ')})`); });
    const activeLogs = new Set();
    if (allowedLogs.has('all')) {
        if (allowedLogs.size > 1)
            abort('Cannot allow all logs and other logs at the same time.');
    ...
    if (blockedLogs.has('all')) {
        if (blockedLogs.size > 1)
            abort('Cannot block all logs and other logs at the same time.');
        logTypes.forEach(t => blockedLogs.add(t));
        activeLogs.clear();
    }
    else if (blockedLogs.size) {
        logTypes.forEach(t => { if (!blockedLogs.has(t))
            activeLogs.add(t); });
        activeLogs.delete('all');
    }
    if (!activeLogs.size && !blockedLogs.has('all'))
        logTypes.forEach(t => activeLogs.add(t));
    if (!rpxPathRaw)
        abort('No RPX/ELF file provided! The first positional argument must be the path to the RPX/ELF file to launch.');
    if (!cemu)
        abort('No Cemu path provided! The CEMU_ROOT environment variable is not set, and no path was provided with the --cemu option.');
    if (!game)
        abort('No game path provided! The TACHYON_DEFAULT_GAME_ROOT environment variable is not set, and no path was provided with the --game option.');
    ...
    if (!fs.existsSync(rpxPath))
        abort('RPX/ELF file not found!');
    if (!fs.existsSync(gamePath))
        abort('Game path not found!');
    if (!fs.existsSync(cemuPath))
        abort('Cemu path not found!');
    if (!fs.existsSync(cemuExe))
        abort(`Cemu path is invalid! No Cemu.exe found at the given folder! (Path searched: ${cemuExe})`);
    ...
    ['code', 'meta', 'content'].forEach(folder => {
        if (!gameFolders.includes(folder))
            abort(`Game path is invalid! No "${folder}" folder found in the given folder!` +
    ...
    if (fs.existsSync(cosxmlPath)) {
        ...
        if (matches) {
            ...
            console.info('Preferred RPX filename not found in cos.xml, no action required.');
    }
    process.once('SIGINT', () => {
    ...
    cemuProc.on('spawn', () => {
        ...
        if (!cemuInfo)
            cleanup(), abort('Failed to get Cemu process info!');
        ...
        if (cemuWindowTitle === 'N/A' || !cemuWindowTitle)
            cleanup(), abort('Failed to get Cemu window title!');
        if (!cemuWindowTitle.startsWith('Cemu'))
            cleanup(), abort(`Failed to properly launch Cemu! This likely a Tachyon bug, please report it! (Window title was: ${cemuWindowTitle})`);
        if (cemuWindowTitle.startsWith('Cemu 2'))
            cleanup(), cemuProc.kill(), abort('Cemu 2.0+ is not supported! Please use Cemu 1.26.2f or lower.');
        if (cemuWindowTitle.startsWith('Cemu 1.27'))
            cleanup(), cemuProc.kill(), abort('Cemu 1.27 is not supported! Please use Cemu 1.26.2f or lower.');
    ...
    cemuProc.once('exit', code => {
        if (code)
            console.error(`Cemu exited with code ${code ?? 'null'}`);
        else
            console.success(`${$.bold('[+]')} Cemu exited gracefully.`);
        cleanup(!!code);
    });
    let tail = null;
    let crashlogMode = false;
    const logPath = path.join(cemuPath, 'log.txt');
    function onGameRunning(finalWindowTitle) {
        console.success(`${$.bold('[+]')} Game running!`);
        const matches = finalWindowTitle.match(/FPS: [\d.,]+? \[([\w -]+?)\] \[([\w -]+?)\] \[TitleId: ([\dA-Fa-f]+?-??[\dA-Fa-f]+?)\] (.+?) \[(\w+?) (v\d+?)\]/);
        if (!matches)
            console.warn('Failed to parse Cemu & game info from window title!');
        else {
            if (process.env.TACHYON_DEBUG) {
                const [, graphicAPI, gpuVendor, titleID, gameName, gameRegion, gameVersion] = matches;
        ...
        tail.on('line', (data) => {
            if (data.includes('Crashlog for Cemu')) {
                if (!activeLogs.has('crashlog'))
                    console.error(`Cemu has crashed! (Crashlog suppressed due to your current logging settings)`);
                crashlogMode = true;
            }
            if (!crashlogMode) {
                if (activeLogs.has('all'))
                    return console.log(`${cemuPrefix} ${data}`);
                const timeless = /^\[\d\d:\d\d:\d\d\] /.test(data) ? data.slice(11) : data;
                if (timeless.trim() === '------- Run title -------')
                    pastInitialLogs = true;
                if (!activeLogs.has('init') && !pastInitialLogs)
                    return;
                if (!activeLogs.has('time'))
                    data = timeless;
                if (!activeLogs.has('vulkan') && timeless.startsWith('Vulkan-Info: '))
                    return;
                //if (!activeLogs.has('opengl') && timeless.startsWith('OpenGL-Info: ')) return;
                if (!activeLogs.has('iosu') && timeless.startsWith('IOSU_'))
                    return;
                if (timeless.startsWith('OSConsoleWrite: ')) {
                    if (!activeLogs.has('osconsole'))
                        return;
                    if (!activeLogs.has('osprefix')) {
                        if (data === timeless)
                            data = data.slice(16);
                        else
                            data = data.slice(0, 11) + data.slice(27);
                    }
                }
