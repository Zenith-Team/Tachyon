...
import { DataSink } from 'rpxlib';
import { abort, hex } from './utils.js';
export function generateCafeloaderFiles(oFile, patches, map) {
    ...
    if (!fs.existsSync('out'))
        fs.mkdirSync('out');
    if (!fs.existsSync(cafeloaderDir))
        fs.mkdirSync(cafeloaderDir);
    ...
    function generateCodeAndDataBin() {
        ...
        if (fs.existsSync(dataBinPath))
            fs.rmSync(dataBinPath);
        const datafd = fs.openSync(dataBinPath, 'a');
        if (sectionText?.hasData)
            fs.writeFileSync(codeBinPath, sectionText.data);
        if (sectionRodata?.hasData)
            fs.appendFileSync(datafd, sectionRodata.data);
        if (sectionData?.hasData)
            fs.appendFileSync(datafd, sectionData.data);
    }
