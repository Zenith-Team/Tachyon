...
export class Project {
    constructor(projectPath, metaPath, ghsPath, target, consoleOutput) {
        this.modules = [];
        this.defines = [];
        this.cppFiles = [];
        this.asmFiles = [];
        ...
        try {
            const yaml = yamlLib.parse(fs.readFileSync(path.join(metaPath, 'project.yaml'), 'utf8'));
            this.name = yaml.Name;
            ...
            if (target.startsWith('Template/'))
                abort('Cannot directly build a template target.');
            if (target in yaml.Targets) {
                ...
                if (tgt.Extends) {
                    ...
                    if (templateName in yaml.Targets) {
                        ...
                        if (template.Extends)
                            abort(`Template target ${templateName} cannot extend other template targets.`);
                        ...
                        abort(`Target ${target} extends unknown template ${tgt.Extends}.`);
                }
                else {
                    tgt.AddrMap ??= target;
                    tgt.BaseRpx ??= target;
                ...
                if (consoleOutput !== 'none')
                    this.targetAddrMap += `-${consoleOutput}`;
            }
            else
                abort(`Target ${target} not found on project.yaml!`);
        }
        catch {
            abort('Invalid project.yaml!');
        }
        // Verify tools
        if (!fs.existsSync(path.join(this.ghsPath, 'gbuild.exe')) ||
    ...
    createGPJ(consoleOutput, extraCompilerFlags) {
...
\t-I${path.relative(this.meta, this.includeDir)}`);
        if (consoleOutput !== 'none')
            gpj.push(`\t-DCONSOLE=${consoleOutput}`);
        for (const define of this.defines)
            gpj.push(`\t-D${define}`);
        for (const cpp of this.cppFiles)
            gpj.push(path.join(path.relative(this.meta, this.sourceDir), cpp));
        fs.writeFileSync(path.join(this.meta, 'project.gpj'), gpj.join('\n'));
    }
    link(map, extraLinkerFlags) {
...
\t.bss    : > data
        ...
        let objFiles = [];
        for (const cppfile of this.cppFiles)
            objFiles.push(cppfile.replace('.cpp', '.o'));
        for (const asmfile of this.asmFiles)
        ...
        if (elxr.error || elxr.signal || elxr.stderr || elxr.status !== 0)
            abort('exlr command failed!');
    }
    patches() {
        let output = [];
        for (const module of this.modules) {
